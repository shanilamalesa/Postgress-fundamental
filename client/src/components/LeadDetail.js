...
import { getLead, updateLead, claimLead, getUsers, reassignLead } from "../services/api";
...
export default function LeadDetail({ leadId, onClose, onUpdated }) {
  ...
  async function claimLeadHandler() {
    try {
      setError(null);
        ...
        {lead && (
          <>
            <div className="mt-4 space-y-1 text-sm">
              <div><span className="text-gray-500">Phone:</span> +{lead.wa_phone}</div>
              <div><span className="text-gray-500">Email:</span> {lead.email || "-"}</div>
              <div><span className="text-gray-500">Inquiry:</span> {lead.inquiry_type || "-"}</div>
              <div><span className="text-gray-500">Created:</span> {lead.created_at}</div>
              <div><span className="text-gray-500">Assigned to:</span> {lead.assigned_to_name || "Unassigned"}</div>
            </div>
            ...
            <div className="mt-8">
              ...
              <div className="space-y-2">
                {(lead.messages || []).map((m) => (
                  ...
                  >
                    <div>{m.body}</div>
                    <div className={"text-[10px] mt-1 " + (m.direction === "inbound" ? "text-gray-500" : "text-blue-100")}>
                      {m.created_at}
                    </div>
                  </div>
                ))}
